...
def KMeans_given_Initial_Centroids(
    dataframe: pandas.DataFrame, centroids: numpy.array
):
...
def KMeans_divided_dataset(
    ...
    original_centroids = sorted(
        [list(triple) for triple in original_centroids]
    )

    train_x = train.iloc[:, 5:8].values
    k_means_train = KMeans(
        n_clusters=7,
        init="k-means++",
        random_state=50,
        tol=1e-8,
    ...
    train_centroids = sorted(
        [list(triple) for triple in train_centroids]
    )
